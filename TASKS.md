...
|---|---|---|---|---|
| `S1-A1` | todo | Scrape/transcribe the **full UNS PMF BSc Informatics** curriculum into `data/curricula/uns-pmf-informatics-bsc.json` (both modules, English titles, descriptions, outcomes, ECTS). Replaces the sample file. | `GET /api/v1/programmes` reports >= 45 courses and every course has a non-empty `description` | - |
